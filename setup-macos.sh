...
# Automates configuration of macOS system settings for fresh installs
# Run this script after initial setup to configure system preferences
...
# Sleep the display after 30 minutes on AC power
...
# Never go into computer sleep mode on AC power
...
# Enable Firewall Stealth Mode (don't respond to ICMP ping requests or connection attempts from closed TCP/UDP ports)
sudo /usr/libexec/ApplicationFirewall/socketfilterfw --setstealthmode on

# Enable FileVault (if not already enabled - will prompt for user action)
if [[ $(sudo fdesetup status | grep -c "Off") -eq 1 ]]; then
    echo "FileVault is currently disabled. Enable it in System Preferences > Security & Privacy > FileVault"
fi
